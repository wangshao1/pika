...
namespace storage {
rocksdb::Status Instance::ScanSetsKeyNum(KeyInfo* key_info) {
  uint64_t keys = 0;
  uint64_t expires = 0;
  uint64_t ttl_sum = 0;
...
rocksdb::Status Instance::SetsPKPatternMatchDel(const std::string& pattern, int32_t* ret) {
  rocksdb::ReadOptions iterator_options;
  const rocksdb::Snapshot* snapshot;
  ScopeSnapshot ss(db_, &snapshot);
  iterator_options.snapshot = snapshot;
...
rocksdb::Status Instance::SAdd(const Slice& key, const std::vector<std::string>& members, int32_t* ret) {
  ...
  if (s.ok()) {
    ...
    if (parsed_sets_meta_value.IsStale() || parsed_sets_meta_value.count() == 0) {
      ...
      for (const auto& member : filtered_members) {
        SetsMemberKey sets_member_key(0/*db_id*/, 0/*slot_id*/, key, version, member);
        batch.Put(handles_[4], sets_member_key.Encode(), Slice());
      }
...
rocksdb::Status Instance::SCard(const Slice& key, int32_t* ret) {
  *ret = 0;
  std::string meta_value;
  BaseMetaKey base_meta_key(0/*db_id*/, 0/*slot_id*/, key);
  rocksdb::Status s = db_->Get(default_read_options_, handles_[3], base_meta_key.Encode(), &meta_value);
...
rocksdb::Status Instance::SDiff(const std::vector<std::string>& keys, std::vector<std::string>* members) {
  if (keys.empty()) {
  ...
  BaseMetaKey base_meta_key0(0/*db_id*/, 0/*slot_id*/, keys[0]);
  ...
  if (s.ok()) {
    ...
    if (!parsed_sets_meta_value.IsStale() && parsed_sets_meta_value.count() != 0) {
      ...
      std::string member_value;
      version = parsed_sets_meta_value.version();
      SetsMemberKey sets_member_key(0/*db_id*/, 0/*slot_id*/, keys[0], version, Slice());
      ...
      for (iter->Seek(prefix); iter->Valid() && iter->key().starts_with(prefix); iter->Next()) {
        ...
        for (const auto& key_version : vaild_sets) {
          SetsMemberKey sets_member_key(0/*db_id*/, 0/*slot_id*/, key_version.key, key_version.version, member);
...
rocksdb::Status Instance::SDiffstore(const Slice& destination, const std::vector<std::string>& keys, std::vector<std::string>& value_to_dest, int32_t* ret) {
  if (keys.empty()) {
  ...
  for (const auto& member : members) {
    SetsMemberKey sets_member_key(0/*db_id*/, 0/*slot_id*/, destination, version, member);
    batch.Put(handles_[4], sets_member_key.Encode(), Slice());
  }
...
rocksdb::Status Instance::SInter(const std::vector<std::string>& keys, std::vector<std::string>* members) {
  if (keys.empty()) {
...
rocksdb::Status Instance::SInterstore(const Slice& destination, const std::vector<std::string>& keys, std::vector<std::string>& value_to_dest, int32_t* ret) {
  if (keys.empty()) {
...
rocksdb::Status Instance::SMembers(const Slice& key, std::vector<std::string>* members) {
  rocksdb::ReadOptions read_options;
  const rocksdb::Snapshot* snapshot;
  ...
  if (s.ok()) {
    ...
    if (parsed_sets_meta_value.IsStale()) {
      ...
      version = parsed_sets_meta_value.version();
      SetsMemberKey sets_member_key(0/*db_id*/, 0/*slot_id*/, key, version, Slice());
      Slice prefix = sets_member_key.Encode();
      auto iter = db_->NewIterator(read_options, handles_[4]);
      for (iter->Seek(prefix); iter->Valid() && iter->key().starts_with(prefix); iter->Next()) {
        ParsedSetsMemberKey parsed_sets_member_key(iter->key());
...
rocksdb::Status Instance::SMove(const Slice& source, const Slice& destination, const Slice& member, int32_t* ret) {
  *ret = 0;
  rocksdb::WriteBatch batch;
  rocksdb::ReadOptions read_options;
  ...
  if (s.ok()) {
    ...
    if (parsed_sets_meta_value.IsStale()) {
      ...
      std::string member_value;
      version = parsed_sets_meta_value.version();
      SetsMemberKey sets_member_key(0/*db_id*/, 0/*slot_id*/, source, version, member);
      s = db_->Get(default_read_options_, handles_[4], sets_member_key.Encode(), &member_value);
      if (s.ok()) {
        *ret = 1;
...
rocksdb::Status Instance::SPop(const Slice& key, std::vector<std::string>* members, bool* need_compact, int64_t cnt) {
...
rocksdb::Status Instance::ResetSpopCount(const std::string& key) { return spop_counts_store_->Remove(key); }

rocksdb::Status Instance::AddAndGetSpopCount(const std::string& key, uint64_t* count) {
  size_t old_count = 0;
...
rocksdb::Status Instance::SRandmember(const Slice& key, int32_t count, std::vector<std::string>* members) {
  if (count == 0) {
    return rocksdb::Status::OK();
  }
...
rocksdb::Status Instance::SRem(const Slice& key, const std::vector<std::string>& members, int32_t* ret) {
  *ret = 0;
  rocksdb::WriteBatch batch;
  ScopeRecordLock l(lock_mgr_, key);
...
rocksdb::Status Instance::SUnion(const std::vector<std::string>& keys, std::vector<std::string>* members) {
  if (keys.empty()) {
  ...
  for (const auto& key_version : vaild_sets) {
    SetsMemberKey sets_member_key(0/*db_id*/, 0/*slot_id*/, key_version.key, key_version.version, Slice());
    prefix = sets_member_key.Encode();
    auto iter = db_->NewIterator(read_options, handles_[4]);
    for (iter->Seek(prefix); iter->Valid() && iter->key().starts_with(prefix); iter->Next()) {
      ParsedSetsMemberKey parsed_sets_member_key(iter->key());
...
rocksdb::Status Instance::SUnionstore(const Slice& destination, const std::vector<std::string>& keys, std::vector<std::string>& value_to_dest, int32_t* ret) {
  if (keys.empty()) {
...
rocksdb::Status Instance::SScan(const Slice& key, int64_t cursor, const std::string& pattern, int64_t count,
  ...
  if (s.ok()) {
    ...
    if (parsed_sets_meta_value.IsStale() || parsed_sets_meta_value.count() == 0) {
      *next_cursor = 0;
      ...
      SetsMemberKey sets_member_prefix(0/*db_id*/, 0/*slot_id*/, key, version, sub_member);
      SetsMemberKey sets_member_key(0/*db_id*/, 0/*slot_id*/, key, version, start_point);
...
rocksdb::Status Instance::SetsExpire(const Slice& key, int32_t ttl) {
  std::string meta_value;
  ScopeRecordLock l(lock_mgr_, key);
  BaseMetaKey base_meta_key(0/*db_id*/, 0/*slot_id*/, key);
  rocksdb::Status s = db_->Get(default_read_options_, handles_[3], base_meta_key.Encode(), &meta_value);
...
rocksdb::Status Instance::SetsDel(const Slice& key) {
  std::string meta_value;
  ScopeRecordLock l(lock_mgr_, key);
  BaseMetaKey base_meta_key(0/*db_id*/, 0/*slot_id*/, key);
  rocksdb::Status s = db_->Get(default_read_options_, handles_[3], base_meta_key.Encode(), &meta_value);
...
rocksdb::Status Instance::SetsExpireat(const Slice& key, int32_t timestamp) {
  std::string meta_value;
  ScopeRecordLock l(lock_mgr_, key);
  BaseMetaKey base_meta_key(0/*db_id*/, 0/*slot_id*/, key);
  rocksdb::Status s = db_->Get(default_read_options_, handles_[3], base_meta_key.Encode(), &meta_value);
...
rocksdb::Status Instance::SetsPersist(const Slice& key) {
  std::string meta_value;
  ScopeRecordLock l(lock_mgr_, key);
  BaseMetaKey base_meta_key(0/*db_id*/, 0/*slot_id*/, key);
  rocksdb::Status s = db_->Get(default_read_options_, handles_[3], base_meta_key.Encode(), &meta_value);
...
rocksdb::Status Instance::SetsTTL(const Slice& key, int64_t* timestamp) {
  std::string meta_value;
  BaseMetaKey base_meta_key(0/*db_id*/, 0/*slot_id*/, key);
  rocksdb::Status s = db_->Get(default_read_options_, handles_[3], base_meta_key.Encode(), &meta_value);
  if (s.ok()) {
