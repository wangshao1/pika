...
class ZSetsScoreKeyComparatorImpl : public rocksdb::Comparator {
 public:
  ...
  int Compare(const rocksdb::Slice& a, const rocksdb::Slice& b) const override {
    ...
    p_b += kVersionLength;
    rocksdb::Slice p_a_slice = Slice(ptr_a, std::distance(ptr_a, p_a));
    rocksdb::Slice p_b_slice = Slice(ptr_b, std::distance(ptr_b, p_b));
    int ret = p_a_slice.compare(p_b_slice);
    if (ret != 0) {
      return ret;
    }
    ptr_a = p_a;
